...
GO
/****** Object:  StoredProcedure [dbo].[sp_sys_VentaEntradas_Entradas_Diario_TXT_Cine]    Script Date: 8/1/2025 15:44:20 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER PROCEDURE [dbo].[sp_sys_VentaEntradas_Entradas_Diario_TXT_Cine]
(
   @Id_Evento INT,
   @Fecha_Desde DATE,
   @Fecha_Hasta DATE
...
BEGIN
	...
	, Resumen AS(
	SELECT e.Codigo_Sala, e.Fecha_Calendario, e.Codigo_Pelicula, e.Id_Evento, 
			e.PrecioEntradaFinal as Precio_Venta,
			SUM(e.Cantidad_Entradas) as Cantidad_Entradas
	From EntradasPrecio e
	GROUP BY e.Codigo_Sala, e.Codigo_Pelicula, e.Fecha_Calendario, e.Codigo_Pelicula, e.Id_Evento,  
				e.PrecioEntradaFinal
	...
	ORDER BY r.Codigo_Sala, r.Codigo_Pelicula, r.Precio_Venta ASC
